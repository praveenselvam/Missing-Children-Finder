...
Sahi.prototype.shouldAppend = function (el) {
	return !(this._isFF() || el.readOnly || el.disabled);
}
...
Sahi.prototype.checkRegex = function(s){
	return ((typeof s) == "string" && s.match(this.CHECK_REGEXP)) ?  new RegExp(s.substring(1, s.length-1)) : s;
};
...
Sahi.prototype.isWithinBounds = function (tags){
	...
	for (var i=0; i<tags.length; i++){
		...
		if (this.alignX && !this.withinOffset(this.alignX, xy[0], 5)){
			add = false;
...
Sahi.prototype.check204Response = function(win){
	if (win._sahi.loaded != true) {
		...
		if (was204) win._sahi.loaded = true;
	}
    var frs = win.frames;
    if (frs) {
        for (var j = 0; j < frs.length; j++) {
...
Sahi.prototype._isIE = function () {return navigator.appName == "Microsoft Internet Explorer";}
Sahi.prototype._isFF3 = function () {return /Firefox\/3|Iceweasel\/3|Shiretoko\/3/.test(navigator.userAgent);};
Sahi.prototype._isFF = function () {return /Firefox|Iceweasel|Shiretoko/.test(navigator.userAgent);};
Sahi.prototype._isChrome = function () {return /Chrome/.test(navigator.userAgent);};
Sahi.prototype._isSafari = function () {return /Safari/.test(navigator.userAgent);};
...
Sahi.prototype.isIgnorableId = function(id){
    ...
    return id.match(/^z_/) || id.match(/^j_id/) || id.match(/^ext[-]gen/) || id.match(/^[:]/) || id.match(/^zk_comp_/);
    //  || id.match(/_[0-9]{10,}_/);
};
...
Sahi.prototype.makeLibFunctionsAvailable = function(){
	var fns = ["_scriptName", "_scriptPath", "_suiteInfo", "_userDataDir", "_sessionInfo", "_userDataPath", "_readFile", "_readCSVFile"];
